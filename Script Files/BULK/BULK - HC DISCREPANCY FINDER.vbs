'STATS GATHERING----------------------------------------------------------------------------------------------------
name_of_script = "BULK - HC DISCREPANCY FINDER.vbs"
start_time = timer
...
'Custom Functions not yet added to the FuncLib----------------------------------------------------------------------------------------------------
...
'Dialog----------------------------------------------------------------------------------------------------
BeginDialog HC_discrepancy_finder_dialog, 0, 0, 281, 110, "HC Discrepancy Finder"
  EditBox 75, 45, 200, 15, x_number
  CheckBox 10, 90, 145, 10, "Check here to run this query county-wide.", all_workers_check
  ButtonGroup ButtonPressed
    OkButton 170, 85, 50, 15
    CancelButton 225, 85, 50, 15
  Text 10, 50, 65, 10, "Worker(s) to check:"
  Text 20, 20, 255, 20, "NOTE: running queries county-wide can take a significant amount of resources and time. This should be done after hours."
  Text 15, 5, 255, 10, "***This script will review HC coding in MAXIS and MMIS to ensure accuracy***"
  Text 10, 70, 265, 10, "Enter 7 digits of your workers' x1 numbers (ex: x######), separated by a comma."
EndDialog
...
DO										
	DO									
		...
		dialog HC_discrepancy_finder_dialog				'main dialog'				
		If buttonpressed = 0 THEN stopscript	'script ends if cancel is selected'							
		IF len(x_number) <> 7 THEN err_msg = err_msg & vbCr & "* The X number must be the full 7 digits."					'mandatory field									
		IF err_msg <> "" THEN MsgBox "*** NOTICE!!! ***" & vbNewLine & err_msg & vbNewLine		'error message including instruction on what needs to be fixed from each mandatory field if incorrect						
	LOOP UNTIL err_msg = ""									'loops until all errors are resolved
	CALL check_for_password(are_we_passworded_out)			'function that checks to ensure that the user has not passworded out of MAXIS, allows user to password back into MAXIS						
Loop until are_we_passworded_out = false					'loops until user passwords back in					

'navigating to REPT/ACTV & checking the x_number 
back_to_SELF
